...
def run_demo() -> dict:
    ...
    # Build a compact per-conflict summary for the detail line
    def _friendly_metric(m: str) -> str:
        """Convert internal metric identifiers to business-friendly labels."""
        if "~" in m:
            parts = m.split("~", 1)
            return f"{parts[0].replace('_', ' ').title()} vs {parts[1].replace('_', ' ').title()}"
        return m.replace("_", " ").title()

    conflict_lines = "; ".join(
        f"{_friendly_metric(c.metric)} [{c.severity}]" for c in conflicts
    )
    steps.append(
        Step(
            "Conflict Detector", "🔍",
            f"{len(conflicts)} semantic conflicts identified",
            f"{n_contra} silent contradiction(s) and {n_syn} hidden synonym(s) "
            f"detected across {len({d.domain for c in conflicts for d in c.definitions})} "
            f"business domains. Conflicts: {conflict_lines}.",
    ...
    for c in conflicts:
        ...
        c.severity = _severity(c.blast_radius)   # re-score after transitive walk
    ...
    for c in conflicts:
        p = draft_proposal(c)
        proposals.append(p)
        def _fmt_owner(urn: str) -> str:
            """Convert a DataHub owner URN to a readable team name."""
            if not urn:
                return ""
            for prefix in (":corpGroup:", ":corpuser:", ":corpUser:"):
                if prefix in urn:
                    slug = urn.split(prefix)[-1]
                    return slug.replace("-", " ").replace("_", " ").title()
            return urn

        named_approvers = [n for n in (_fmt_owner(o) for o in p.approvers) if n]
        steps.append(
            Step(
                "Reconciliation Broker", "🤝",
                f"Drafting canonical definition: {p.display_name}",
                f"Proposed one canonical definition. Routing to "
                f"{'1 team' if len(named_approvers) == 1 else f'{len(named_approvers)} teams'} "
                f"for approval: {', '.join(named_approvers)}.",
