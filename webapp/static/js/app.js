// Rosetta Enterprise AI Data Intelligence UI
// Runs the five-agent pipeline, animates discovery steps, renders findings
// with AI explanation structure, draws the blast-radius graph, and drives
// the Executive Dashboard with animated score rings.
...
/** Convert internal metric identifiers to business-friendly display labels.
 *  "customer_churn~attrition" → "Customer Churn vs Customer Attrition"
 *  "active_user"              → "active_user"  (unchanged — already readable)
 */
...
/* ──────────────────────────────────────────────────────────────────────────
   Journey nav — highlight whichever stage is active
   ────────────────────────────────────────────────────────────────────────── */
function setJourneyStep(step) {
  const nav = $("#journeyNav");
  nav.hidden = false;
  nav.querySelectorAll(".jstep").forEach((el) => {
...
function animateRing(ringId, score) {
  ...
  if (!ring) return;
  // We need to wait a frame so the transition fires after the display change
...
function renderDashboard(dash) {
  const section = $("#sectionIntelligence");
  ...
  // Score rings — animate number and ring
  ...
  [
    ...
    if (el) animateCount(el, Math.round(val || 0));
    animateRing(ringId, val || 0);
  });

  // Dash stats
  const crit = (dash.critical_risks || []).filter((r) => r.severity === "critical");
  animateCount($("#dsCritRisks"), dash.critical_risks ? dash.critical_risks.length : 0);
  ...
  (dash.critical_risks || []).forEach((r) => {
    ...
    div.innerHTML = `
      <div class="cri-metric">
        <span>${esc(r.metric)}</span>
        <span class="badge ${esc(r.severity)}">${esc(r.severity.toUpperCase())}</span>
        <span style="color:var(--muted);font-size:11px;font-family:monospace;font-weight:400">
  ...
  if (!dash.critical_risks || !dash.critical_risks.length) {
    riskList.innerHTML = `<div style="color:var(--low);font-size:13px">✓ No critical or high-severity risks detected.</div>`;
  }
...
/* ──────────────────────────────────────────────────────────────────────────
   Blast-radius graph (no dependencies)
   ────────────────────────────────────────────────────────────────────────── */
const TYPE_COLOR = {
...
/* ──────────────────────────────────────────────────────────────────────────
   AI Explanation block (Finding → Evidence → Impact → Recommendation)
   ────────────────────────────────────────────────────────────────────────── */
function renderAiExplanation(c) {
  const ai = c.ai_explanation;
  if (!ai) return "";
  return `
    <div class="ai-explain">
      <div class="ai-explain-title">✦ AI Explanation</div>
      <div class="ai-row">
        <span class="ai-label">Finding</span>
        <span class="ai-val finding">${esc(ai.finding)}</span>
      </div>
      <div class="ai-row">
        <span class="ai-label">Evidence</span>
        <span class="ai-val evidence">${esc(ai.evidence)}</span>
      </div>
      <div class="ai-row">
        <span class="ai-label">Impact</span>
        <span class="ai-val impact">${esc(ai.impact)}</span>
      </div>
      <div class="ai-row">
        <span class="ai-label">Recommend</span>
        <span class="ai-val recommendation">${esc(ai.recommendation)}</span>
      </div>
    </div>`;
}
...
function renderConflicts(conflicts) {
  ...
  // Auto-draw top conflict graph and insert a discovery prompt beneath it
  if (conflicts[0] && conflicts[0].impact_graph) {
    drawGraph(conflicts[0].impact_graph, conflicts[0].metric);
    const cue = document.createElement("div");
    cue.className = "graph-cue";
    cue.innerHTML = `<button class="graph-cue-btn" id="scrollToGraph">
      ↓ See how <b>${esc(friendlyMetric(conflicts[0].metric))}</b> contaminates
      ${conflicts[0].blast_radius} downstream assets
    </button>`;
    wrap.insertBefore(cue, wrap.children[1] || null);
...
async function run(endpoint) {
  ...
  try {
    ...
    // Intelligence: executive dashboard (fetch independently)
    const dashRes = await fetch("/api/dashboard");
    const dash = await dashRes.json();
    renderDashboard(dash);
    setJourneyStep("intelligence");

    // Resolution: findings
    renderSummary(report.summary);
    renderConflicts(report.conflicts);
    $("#exportBar").hidden = false;
    setJourneyStep("resolution");

    // Scroll to dashboard
    $("#sectionIntelligence").scrollIntoView({ behavior: "smooth", block: "start" });
  } catch (e) {
